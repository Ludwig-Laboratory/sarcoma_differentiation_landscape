...
# SCTransform normalization (warning: takes a while, and requires significant RAM/swap ~80 gb ?, clear RAM first or may crash otherwise)
# also, preferred SCT normalization splits object by batches and performs SCT on each batch separate; more accurate model of experimental variation, faster, less memory
...
DimPlot(obj, group.by = 'tumor_id')

# clustering
obj <- FindNeighbors(obj, k.param = 100)
obj <- FindClusters(obj, algorithm = 1, resolution  = 0.1)
DimPlot(obj, group.by = c('tumor_id', 'seurat_clusters'))
...
library(harmony)
obj <- RunHarmony(obj, group.by.vars = 'tumor_id', assay.use = 'SCT',
...
DimPlot(obj, group.by = 'tumor_id', reduction = 'umap') +
  DimPlot(obj, group.by = 'tumor_id', reduction = 'umap_harmony')
DimPlot(obj, group.by = 'seurat_clusters', reduction = 'umap') +
  DimPlot(obj, group.by = 'seurat_clusters', reduction = 'umap_harmony')
...
VlnPlot(obj, features=fs, group.by = 'tumor_id')

FeaturePlot(obj, features = c("POU5F1", "NANOG", "SOX9", "PDGFRA"), label=T, reduction='umap_harmony') # iPSC, Scl markers
FeaturePlot(obj, features = c("COL2A1", "ACAN", "MATN4", "COL9A3"), label=T, reduction='umap_harmony') # chondro markers
FeaturePlot(obj, features = c("SOX2", "OTX1", "PAX6", "MITF"), label=T, reduction='umap_harmony') # non-chondro (neuro/melano) markers
FeaturePlot(obj, features = c("TUBA1B", "HIST1H4C", "STMN1", "MCM5"), label=T, reduction='umap_harmony')
DimPlot(obj, group.by = "Phase", reduction='umap_harmony')
